/**
 **  Macro to run the online for the ALPIDE silicon detectors
 **
...
typedef struct EXT_STR_h101_t {
  ...
  EXT_STR_h101_ALPIDE_onion_t alpide;
} EXT_STR_h101;

void online_alpide(const Int_t nev = -1) {
  TStopwatch timer;
  timer.Start();
  ...
  Bool_t fAlpide = true; // ALPIDE detectors
  Bool_t NOTstoremappeddata =
      true; // if true, don't store mapped data in the root file
  Bool_t NOTstorecaldata =
      true; // if true, don't store cal data in the root file
  Bool_t NOTstorehitdata =
      true; // if true, don't store hit data in the root file
  ...
  if (fAlpide) {
    auto unpackalpide =
        new R3BAlpideReader((EXT_STR_h101_ALPIDE_onion *)&ucesb_struct.alpide,
                            offsetof(EXT_STR_h101, alpide));
    source->AddReader(unpackalpide);
  }
  ...
  if (Cal_level) {
    ...
    if (fAlpide) {
      auto alpidemap2cal = new R3BAlpideMapped2Cal();
      run->AddTask(alpidemap2cal);

      auto alpidecal2hit = new R3BAlpideCal2Hit();
      run->AddTask(alpidecal2hit);

      // Add online tasks
      auto r3bOnlineSpectra = new R3BAlpideOnlineSpectra();
      run->AddTask(r3bOnlineSpectra);

      auto alpidecor12 = new R3BSingleAlpideCorrelationOnlineSpectra(
          "CorrelationOnline1_2", 1, "1", 1, "2", 2);
      run->AddTask(alpidecor12);

      auto alpidecor23 = new R3BSingleAlpideCorrelationOnlineSpectra(
          "CorrelationOnline2_3", 1, "2", 2, "3", 3);
      run->AddTask(alpidecor23);

      auto alpidecor34 = new R3BSingleAlpideCorrelationOnlineSpectra(
          "CorrelationOnline3_4", 1, "3", 3, "4", 4);
      run->AddTask(alpidecor34);

      auto alpidecor45 = new R3BSingleAlpideCorrelationOnlineSpectra(
          "CorrelationOnline4_5", 1, "4", 4, "5", 5);
      run->AddTask(alpidecor45);

      auto alpidecor56 = new R3BSingleAlpideCorrelationOnlineSpectra(
          "CorrelationOnline5_6", 1, "5", 5, "6", 6);
      run->AddTask(alpidecor56);
    }
